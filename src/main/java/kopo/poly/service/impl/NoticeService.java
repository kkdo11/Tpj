...
//성길씨
        ...
        List<NoticeDTO> result = noticeMapper.getNoticeList();

        if (result.isEmpty()) {
            return new ArrayList<>();
        }
        List<NoticeDTO> filteringLNoticeList = new ArrayList<>();
        result.forEach(noticeDTO -> {
            if (noticeDTO.getNotice_yn().equals("Y")) {
                noticeDTO.setTitle("[공지] " + noticeDTO.getTitle());
            }

            filteringLNoticeList.add(noticeDTO);
        });


        return filteringLNoticeList;
